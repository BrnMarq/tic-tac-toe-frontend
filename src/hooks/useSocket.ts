...
export const useSocket = () => {
	...
	const makeMove = (position: number) => {
		if (socketRef.current && currentRoom) {
			...
			console.log(
				" No se puede hacer movimiento - socket o currentRoom no disponibles"
			);
		}
	};
	...
	useEffect(() => {
		...
		socket.on(
			"game-over",
			(resultData: { winner: WinnerStatus; winningLine: number[] | null }) => {
				...
				toast({
					...
					description: `Ganador: ${resultData.winner}`,
					duration: 5000,
				});
			}
		);
		...
		socket.on("player-left", () => {
			toast({
				...
				description: "El otro jugador ha salido. Volviendo al menú.",
				duration: 5000,
			});
			leaveRoom();
		});
